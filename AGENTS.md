...
- E2 started. Its four original per-tool reliability items are implemented and live-verified: `RefreshBlueprintCallSites`, `SaveValidatedBlueprints`, `WireSelectionWorkflow`, and `SaveAllDirtyPackages`. See ROADMAP.md's E2 section for exactly what each now guarantees.
...
- The compile-failure rollback branch is now tested by `UnrealMCP.Blueprint.Authoring.CallSiteRollback`. The reconstruction/compile/rollback engine lives in `RefreshBlueprintCallSitesEngine.{h,cpp}` and the tool is the index query over it; that split exists because the SQLite index is built from `AssetRegistry.GetAllAssets` and never sees transient automation fixtures, so a test can only reach the engine directly. Keep index discovery out of the engine.
- That test engineers a real failure rather than injecting one: remove a function parameter while a caller still has the pin linked, and reconstruction orphans the pin so the caller genuinely fails to compile. Prefer this over adding test-only fields to a public schema. It needs `AddExpectedError` for the compiler's orphaned-pin message, otherwise the framework counts the intended failure as a test error.
- E2 mutation audit (partial, see ROADMAP for scope): all 119 tools checked for unguarded object loading — clean. Three fixes landed from it: `SpliceBlueprintExecFlow` now requires `confirm` (it breaks an existing execution link, but its name does not look destructive), `SaveBlueprint` refuses a not-up-to-date compile by default with structured `blueprint_compile_not_up_to_date` recovery evidence and measures `isDirty` rather than hardcoding it, and the adapter's `IsMutationTool` gained an explicit name list because prefixes missed `BuildProjectIndex` and `ValidateBlueprint`.
- Adding a `confirm` gate to an existing tool is a breaking contract change: `UnrealMCP.Blueprint.Authoring.ExecSplice.Live` failed until it was updated to pass `confirm=true`. Expect and check for that.
- `UnrealMCP.Blueprint.Authoring.SaveBlueprint.Live` covers guarded refusal, explicit override, a real temporary save, measured dirty state, schema and exact fixture cleanup. It, `CallSiteRollback`, and `ExecSplice.Live` each pass against the final rebuilt DLL with zero errors and warnings. Test fixtures register/unregister with the Asset Registry and are removed without leaving saved files.
- Final follow-up verification: canonical `UE_544_MCPEditor Win64 Development` build passed, the isolated adapter Release build passed, and all 49 simulated-pipe adapter checks passed. The added adapter checks prove classification, schema operation IDs, forwarding and uncertain-timeout behavior for both `BuildProjectIndex` and `ValidateBlueprint`.
- The normal checkout adapter executable remains locked by two active MCP client processes, so the verified adapter build is in `%LocalAppData%/Temp/UnrealMCPFix_20260912`. Restart configured MCP clients before rebuilding the normal Release output; do not terminate them merely to replace the binary.
- `Adapter/BuildAdapter.bat` now gives users a double-clickable canonical Release build plus `help` and `doctor --json` smoke checks. Close every agent/client using the adapter first; pass `--no-pause` for terminal or automated use.
- Remaining E2 work: idempotency review across mutating tools, and disconnection/replay/expired-ID coverage.
- The earlier unsaved `/Game/A_BadActor.A_BadActor` test change was discarded by the graceful no-save shutdown and was not written to disk. The rebuilt `UE_544_MCP` Editor is open and attached. The `work_dsm` checkout was not touched or synchronized. No PIE or index schema rebuild was run.
