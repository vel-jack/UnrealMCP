...
FDeleteBlueprintNodeTool::FDeleteBlueprintNodeTool()
    : FMCPToolBase(TEXT("DeleteBlueprintNode"), TEXT("Deletes one Blueprint graph node by stable GUID. Requires confirm=true, supports dry-run, and reports affected pin links.")) {}

UnrealMCP::FMCPResponse FDeleteBlueprintNodeTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    FString O, G, GG, N;
    ...
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InvalidParams, TEXT("DeleteBlueprintNode requires objectPath, nodeGuid, and graphName or graphGuid."));
    }
    ...
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InvalidParams, TEXT("DeleteBlueprintNode requires graphName or graphGuid."));
    }
    ...
    if (!Dry && !Confirm)
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InvalidParams, TEXT("DeleteBlueprintNode requires confirm=true unless dryRun=true."));
    }

    int32 Links = 0;
    FString Title, File, IdxErr, ExecErr;
    bool Idx = false;
    ...
    bool Ok = UnrealMCP::BlueprintToolUtils::ExecuteOnGameThreadSync([&](FString& E)
    {
        ...
        const FScopedTransaction T(NSLOCTEXT("UnrealMCP", "DeleteBlueprintNode", "UnrealMCP Delete Blueprint Node"));
        B->Modify();
        Graph->Modify();
        Node->Modify();
        FBlueprintEditorUtils::RemoveNode(B, Node, true);
    ...
    J->SetNumberField(TEXT("affectedPinLinkCount"), Links);
    J->SetBoolField(TEXT("dryRun"), Dry);
    J->SetBoolField(TEXT("deleted"), !Dry);
...
TSharedPtr<FJsonObject> FDeleteBlueprintNodeTool::BuildInputSchema() const
{
    ...
    P->SetObjectField(TEXT("nodeGuid"), BuildStringProperty(TEXT("Exact node GUID.")));
    P->SetObjectField(TEXT("confirm"), BuildBoolProperty(TEXT("Required for deletion.")));
    P->SetObjectField(TEXT("dryRun"), BuildBoolProperty(TEXT("Report impact without deletion.")));
    P->SetObjectField(TEXT("saveAfterEdit"), BuildBoolProperty(TEXT("Save and refresh after deletion.")));
