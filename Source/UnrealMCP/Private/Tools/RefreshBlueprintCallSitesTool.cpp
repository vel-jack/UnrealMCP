    ...
    for (const TPair<FString, TArray<FCallSiteRow>>& Entry : RowsByBlueprint)
    {
        ...
        const bool bAssetSucceeded = BlueprintToolUtils::ExecuteOnGameThreadSync(
            [&](FString& OutError) -> bool
            {
                ...
                // Resolve every indexed row up front in both modes. A row that no longer resolves is a
                // real caller failure (usually a stale index), not something to silently skip.
                auto ResolveRows = [&](TArray<UK2Node_CallFunction*>& OutNodes, TArray<UEdGraph*>& OutGraphs) -> bool
                {
                    bool bAllFound = true;
                ...
                const bool bAllResolved = ResolveRows(CallNodes, Graphs);
        ...
        AssetResult->SetBoolField(TEXT("succeeded"), bAssetSucceeded);
        AssetResult->SetArrayField(TEXT("nodesReconstructed"), NodeResults);
        AssetResult->SetBoolField(TEXT("compiled"), bCompiled);
        AssetResult->SetBoolField(TEXT("compileSkipped"), bCompileSkipped);
        ...
        AssetResult->SetBoolField(TEXT("restorationVerified"), bRestorationVerified);
        AssetResult->SetBoolField(TEXT("graphRestored"), bGraphRestored);
        AssetResult->SetBoolField(TEXT("dirtyRestored"), bDirtyRestored);
