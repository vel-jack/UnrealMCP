...
UnrealMCP::FMCPResponse FCompileAllBlueprintsTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    ...
    if (!bSucceeded)
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InternalError, ExecutionError.IsEmpty() ? TEXT("CompileAllBlueprints failed on the game thread.") : ExecutionError);
    }
