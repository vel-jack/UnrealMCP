...
UnrealMCP::FMCPResponse FAddBlueprintVariableTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    ...
    Request.Params->TryGetStringField(TEXT("containerType"), ContainerTypeName); Request.Params->TryGetStringField(TEXT("valueType"), ValueTypeName); Request.Params->TryGetStringField(TEXT("valueTypeObjectPath"), ValueTypeObjectPath);
    ...
    if(ContainerTypeName.IsEmpty())ContainerTypeName=bArray?TEXT("array"):TEXT("none");
    if(bArray&&!ContainerTypeName.Equals(TEXT("array"),ESearchCase::IgnoreCase))return BuildError(Request,UnrealMCP::EMCPErrorCode::InvalidParams,TEXT("isArray=true conflicts with a non-array containerType."));
    ...
    const bool bSucceeded=UnrealMCP::BlueprintToolUtils::ExecuteOnGameThreadSync([&](FString& OutError)
    {
        ...
        FEdGraphPinType PinType; if(!UnrealMCP::BlueprintEditToolUtils::BuildContainerPinType(TypeName,TypeObjectPath,ContainerTypeName,ValueTypeName,ValueTypeObjectPath,PinType,OutError))return false;
    ...
    UnrealMCP::FMCPResponse Response;Response.Id=Request.Id;TSharedRef<FJsonObject>Result=BuildBooleanResult(true);Result->SetStringField(TEXT("objectPath"),ObjectPath);Result->SetStringField(TEXT("variableName"),VariableName);Result->SetStringField(TEXT("type"),TypeName);Result->SetStringField(TEXT("typeObjectPath"),TypeObjectPath);Result->SetStringField(TEXT("containerType"),ContainerTypeName.ToLower());Result->SetStringField(TEXT("valueType"),ValueTypeName);Result->SetStringField(TEXT("valueTypeObjectPath"),ValueTypeObjectPath);Result->SetBoolField(TEXT("isArray"),ContainerTypeName.Equals(TEXT("array"),ESearchCase::IgnoreCase));Result->SetBoolField(TEXT("instanceEditable"),bInstanceEditable);Result->SetBoolField(TEXT("dryRun"),bDryRun);Result->SetBoolField(TEXT("alreadyExists"),bAlreadyExists);Result->SetBoolField(TEXT("added"),!bDryRun&&!bAlreadyExists);Result->SetBoolField(TEXT("compiled"),bCompile&&!bDryRun&&!bAlreadyExists);Result->SetBoolField(TEXT("compileSucceeded"),bCompileSucceeded);Result->SetBoolField(TEXT("saved"),bSave&&!bDryRun&&!bAlreadyExists);Result->SetStringField(TEXT("savedFilename"),SavedFilename);Result->SetBoolField(TEXT("indexRefreshed"),bIndexRefreshed);Result->SetStringField(TEXT("indexRefreshError"),IndexRefreshError);Response.Result=Result;return Response;
}
...
TSharedPtr<FJsonObject> FAddBlueprintVariableTool::BuildInputSchema() const
{
    ...
    P->SetObjectField(TEXT("objectPath"),BuildStringProperty(TEXT("Target Blueprint object path.")));P->SetObjectField(TEXT("variableName"),BuildStringProperty(TEXT("Unique member variable name.")));P->SetObjectField(TEXT("type"),BuildStringProperty(TEXT("Scalar type, or Map key type.")));P->SetObjectField(TEXT("typeObjectPath"),BuildStringProperty(TEXT("Required class path for object/class types.")));P->SetObjectField(TEXT("containerType"),BuildStringProperty(TEXT("none, array, set, or map.")));P->SetObjectField(TEXT("valueType"),BuildStringProperty(TEXT("Required Map value type.")));P->SetObjectField(TEXT("valueTypeObjectPath"),BuildStringProperty(TEXT("Class path for object/class Map values.")));P->SetObjectField(TEXT("defaultValue"),BuildStringProperty(TEXT("Optional Unreal-exported default literal.")));P->SetObjectField(TEXT("category"),BuildStringProperty(TEXT("Optional variable category.")));P->SetObjectField(TEXT("isArray"),BuildBoolProperty(TEXT("Backward-compatible alias for containerType=array.")));P->SetObjectField(TEXT("instanceEditable"),BuildBoolProperty(TEXT("Allow editing on Blueprint instances.")));P->SetObjectField(TEXT("dryRun"),BuildBoolProperty(TEXT("Validate without mutation.")));P->SetObjectField(TEXT("compileAfterEdit"),BuildBoolProperty(TEXT("Compile only this Blueprint.")));P->SetObjectField(TEXT("saveAfterEdit"),BuildBoolProperty(TEXT("Save after mutation.")));S->SetObjectField(TEXT("properties"),P);TArray<TSharedPtr<FJsonValue>>R{MakeShared<FJsonValueString>(TEXT("objectPath")),MakeShared<FJsonValueString>(TEXT("variableName")),MakeShared<FJsonValueString>(TEXT("type"))};S->SetArrayField(TEXT("required"),R);return S;
}
