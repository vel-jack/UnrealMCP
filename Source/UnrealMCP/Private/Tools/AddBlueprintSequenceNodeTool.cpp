...
FAddBlueprintSequenceNodeTool::FAddBlueprintSequenceNodeTool()
    : FMCPToolBase(TEXT("AddBlueprintSequenceNode"), TEXT("Adds a collision-aware Sequence node to a stable Blueprint graph. Supports dry-run and optional save.")) {}

UnrealMCP::FMCPResponse FAddBlueprintSequenceNodeTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    FString O, G, GG;
    if (!Request.Params.IsValid() || !Request.Params->TryGetStringField(TEXT("objectPath"), O))
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InvalidParams, TEXT("AddBlueprintSequenceNode requires objectPath and graphName or graphGuid."));
    }
    ...
    bool Ok = UnrealMCP::BlueprintToolUtils::ExecuteOnGameThreadSync([&](FString& E)
    {
        ...
        const FScopedTransaction T(NSLOCTEXT("UnrealMCP", "AddSequence", "UnrealMCP Add Sequence Node"));
        B->Modify();
        Graph->Modify();
        auto* Node = NewObject<UK2Node_ExecutionSequence>(Graph);
    ...
    J->SetStringField(TEXT("nodeType"), TEXT("sequence"));
    ...
    J->SetBoolField(TEXT("autoPlaced"), P.bAutoPlaced);
...
TSharedPtr<FJsonObject> FAddBlueprintSequenceNodeTool::BuildInputSchema() const
{
    ...
    P->SetObjectField(TEXT("dryRun"), BuildBoolProperty(TEXT("Preview without mutation.")));
