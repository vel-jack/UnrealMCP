...
UnrealMCP::FMCPResponse FListBlueprintComponentsTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    ...
    const bool bSucceeded = UnrealMCP::BlueprintToolUtils::ExecuteOnGameThreadSync(
        [&](FString& OutError)
        {
            ...
            {
                OutError = TEXT("ListComponents could not load a Blueprint from params.objectPath.");
                return false;
            }
    ...
    if (!bSucceeded)
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InvalidParams, ExecutionError.IsEmpty() ? TEXT("ListComponents could not load a Blueprint from params.objectPath.") : ExecutionError);
    }
