...
UnrealMCP::FMCPResponse FAddBlueprintBranchNodeTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    ...
    const bool bSucceeded=UnrealMCP::BlueprintToolUtils::ExecuteOnGameThreadSync([&](FString&OutError)
    {
        ...
        Placement=UnrealMCP::BlueprintGraphEditToolUtils::ResolvePlacement(Graph,Request.Params,OutError);if(!OutError.IsEmpty())return false;if(bDryRun){UK2Node_IfThenElse*Preview=UnrealMCP::BlueprintGraphEditToolUtils::CreateBranchNode(Graph,true);if(!Preview){OutError=TEXT("Could not create the detached Branch preview.");return false;}Pins=UnrealMCP::BlueprintGraphEditToolUtils::SerializePins(Preview);return true;}
        const FScopedTransaction Transaction(NSLOCTEXT("UnrealMCP","AddBranchNode","UnrealMCP Add Branch Node"));Blueprint->Modify();Graph->Modify();UK2Node_IfThenElse*Node=UnrealMCP::BlueprintGraphEditToolUtils::CreateBranchNode(Graph,false);if(!Node){OutError=TEXT("Could not create the Branch node.");return false;}UnrealMCP::BlueprintGraphEditToolUtils::PlaceNewNode(Graph,Node,Placement);FBlueprintEditorUtils::MarkBlueprintAsModified(Blueprint);NodeGuid=UnrealMCP::BlueprintGraphEditToolUtils::GetNodeGuid(Node);Pins=UnrealMCP::BlueprintGraphEditToolUtils::SerializePins(Node);
    ...
    UnrealMCP::FMCPResponse Response;Response.Id=Request.Id;TSharedRef<FJsonObject>Result=BuildBooleanResult(true);Result->SetStringField(TEXT("objectPath"),ObjectPath);Result->SetStringField(TEXT("graphName"),GraphName);Result->SetStringField(TEXT("graphGuid"),GraphGuid);Result->SetBoolField(TEXT("dryRun"),bDryRun);Result->SetBoolField(TEXT("pinsPredicted"),bDryRun);Result->SetBoolField(TEXT("added"),!bDryRun);Result->SetStringField(TEXT("nodeGuid"),NodeGuid);Result->SetStringField(TEXT("nodeType"),TEXT("branch"));Result->SetNumberField(TEXT("positionX"),Placement.X);Result->SetNumberField(TEXT("positionY"),Placement.Y);Result->SetBoolField(TEXT("autoPlaced"),Placement.bAutoPlaced);Result->SetBoolField(TEXT("collisionAdjusted"),Placement.bCollisionAdjusted);Result->SetArrayField(TEXT("pins"),Pins);Result->SetBoolField(TEXT("saved"),bSave&&!bDryRun);Result->SetStringField(TEXT("savedFilename"),SavedFilename);Result->SetBoolField(TEXT("indexRefreshed"),bIndexRefreshed);Result->SetStringField(TEXT("indexRefreshError"),IndexError);Response.Result=Result;return Response;
}
