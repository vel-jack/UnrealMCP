...
UnrealMCP::FMCPResponse FListBlueprintGraphsTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    ...
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InvalidParams, TEXT("ListBlueprintGraphs requires objectPath."));
    }

    TArray<TSharedPtr<FJsonValue>> Graphs;
    FString Error;
    const bool bSucceeded = UnrealMCP::IndexedQueryToolUtils::ExecuteWithProjectIndex([&](FSQLiteDatabase& Database, FString& OutError)
    {
        ...
        {
            OutError = TEXT("ListBlueprintGraphs could not prepare its query.");
            return false;
        }

        const int64 Result = Statement.Execute([&](const FSQLitePreparedStatement& Row)
        {
            FString Name, Guid, Type, Entry;
            int32 Nodes = 0;
            if (!Row.GetColumnValueByIndex(0, Name) || !Row.GetColumnValueByIndex(1, Guid) || !Row.GetColumnValueByIndex(2, Type) || !Row.GetColumnValueByIndex(3, Entry) || !Row.GetColumnValueByIndex(4, Nodes))
            {
                return ESQLitePreparedStatementExecuteRowResult::Error;
            }

            TSharedRef<FJsonObject> Item = MakeShared<FJsonObject>();
            Item->SetStringField(TEXT("graphName"), Name);
            Item->SetStringField(TEXT("graphGuid"), Guid);
            Item->SetStringField(TEXT("graphType"), Type);
            Item->SetStringField(TEXT("entryNodeGuid"), Entry);
            Item->SetNumberField(TEXT("nodeCount"), Nodes);
        ...
        {
            OutError = Database.GetLastError().IsEmpty() ? TEXT("ListBlueprintGraphs query failed.") : Database.GetLastError();
            return false;
        }
    ...
    if (!bSucceeded)
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InternalError, Error);
    }
...
TSharedPtr<FJsonObject> FListBlueprintGraphsTool::BuildInputSchema() const
{
    ...
    P->SetObjectField(TEXT("objectPath"), UnrealMCP::BlueprintEditToolUtils::BuildStringProperty(TEXT("Indexed Blueprint object path.")));
