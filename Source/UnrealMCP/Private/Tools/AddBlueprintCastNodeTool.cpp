...
FAddBlueprintCastNodeTool::FAddBlueprintCastNodeTool()
    : FMCPToolBase(TEXT("AddBlueprintCastNode"), TEXT("Adds a typed Dynamic Cast node with optional pure mode, collision-aware placement, dry-run, and optional save.")) {}

UnrealMCP::FMCPResponse FAddBlueprintCastNodeTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    FString O, G, GG, Target;
    if (!Request.Params.IsValid() || !Request.Params->TryGetStringField(TEXT("objectPath"), O) || !Request.Params->TryGetStringField(TEXT("targetClassPath"), Target))
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InvalidParams, TEXT("AddBlueprintCastNode requires objectPath, targetClassPath, and graph selector."));
    }
    ...
    bool Pure = UnrealMCP::BlueprintEditToolUtils::GetOptionalBool(Request.Params, TEXT("pure"), false);

    UnrealMCP::BlueprintGraphEditToolUtils::FPlacement P;
    FString NG, Resolved, File, IdxErr, ExecErr;
    bool Idx = false;
    ...
    bool Ok = UnrealMCP::BlueprintToolUtils::ExecuteOnGameThreadSync([&](FString& E)
    {
        ...
        if (!UnrealMCP::BlueprintEditToolUtils::ResolveClass(Target, TargetClass, E))
        {
            return false;
        }

        if (!TargetClass->IsChildOf<UObject>())
        {
            E = TEXT("targetClassPath must derive from UObject.");
            return false;
        }

        Resolved = TargetClass->GetPathName();
        ...
        const FScopedTransaction T(NSLOCTEXT("UnrealMCP", "AddCast", "UnrealMCP Add Dynamic Cast"));
        B->Modify();
        Graph->Modify();
        auto* Node = NewObject<UK2Node_DynamicCast>(Graph);
    ...
    J->SetStringField(TEXT("targetClassPath"), Resolved);
    J->SetStringField(TEXT("nodeGuid"), NG);
    J->SetBoolField(TEXT("pure"), Pure);
...
TSharedPtr<FJsonObject> FAddBlueprintCastNodeTool::BuildInputSchema() const
{
    ...
    for (const TCHAR* N : { TEXT("objectPath"), TEXT("graphName"), TEXT("graphGuid"), TEXT("targetClassPath"), TEXT("relativeToNodeGuid") })
    {
        P->SetObjectField(N, BuildStringProperty(TEXT("Blueprint graph/class/placement selector.")));
    }
    ...
    P->SetObjectField(TEXT("pure"), BuildBoolProperty(TEXT("Create a pure cast without exec pins.")));
    ...
    TArray<TSharedPtr<FJsonValue>> Q{ MakeShared<FJsonValueString>(TEXT("objectPath")), MakeShared<FJsonValueString>(TEXT("targetClassPath")) };
