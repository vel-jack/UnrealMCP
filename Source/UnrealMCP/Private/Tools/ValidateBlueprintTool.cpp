...
UnrealMCP::FMCPResponse FValidateBlueprintTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    ...
    FString StatusBefore, StatusAfter, ExecutionError;
    ...
    const bool bSucceeded = UnrealMCP::BlueprintToolUtils::ExecuteOnGameThreadSync([&](FString& OutError)
    {
        ...
        WarningCount = Log.NumWarnings;
        StatusAfter = UnrealMCP::BlueprintToolUtils::GetBlueprintStatusString(Blueprint->Status);
        bValid = ErrorCount == 0 && (Blueprint->Status == BS_UpToDate || Blueprint->Status == BS_UpToDateWithWarnings);
        ...
        for (const TSharedRef<FTokenizedMessage>& Message : Log.Messages)
        {
            ...
            Item->SetStringField(TEXT("severity"), Severity == EMessageSeverity::Error
                ? TEXT("error")
                : (Severity == EMessageSeverity::Warning || Severity == EMessageSeverity::PerformanceWarning ? TEXT("warning") : TEXT("info")));
            Item->SetStringField(TEXT("text"), Message->ToText().ToString());
    ...
    TSharedRef<FJsonObject> Result = BuildBooleanResult(bValid);
...
TSharedPtr<FJsonObject> FValidateBlueprintTool::BuildInputSchema() const
{
    ...
    P->SetObjectField(TEXT("objectPath"), UnrealMCP::BlueprintEditToolUtils::BuildStringProperty(TEXT("Blueprint object path.")));
