...
FMoveBlueprintNodeTool::FMoveBlueprintNodeTool()
    : FMCPToolBase(TEXT("MoveBlueprintNode"), TEXT("Moves a Blueprint node using explicit or relative collision-aware placement. Supports dry-run and optional save.")) {}

UnrealMCP::FMCPResponse FMoveBlueprintNodeTool::Execute(const UnrealMCP::FMCPRequest& Request) const
{
    FString O, G, GG, N;
    ...
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InvalidParams, TEXT("MoveBlueprintNode requires objectPath, nodeGuid, and graphName or graphGuid."));
    }
    ...
    {
        return BuildError(Request, UnrealMCP::EMCPErrorCode::InvalidParams, TEXT("MoveBlueprintNode requires graphName or graphGuid."));
    }
    ...
    bool Ok = UnrealMCP::BlueprintToolUtils::ExecuteOnGameThreadSync([&](FString& E)
    {
        ...
        const FScopedTransaction T(NSLOCTEXT("UnrealMCP", "MoveBlueprintNode", "UnrealMCP Move Blueprint Node"));
        B->Modify();
        Graph->Modify();
        Node->Modify();
        Node->NodePosX = P.X;
    ...
    J->SetNumberField(TEXT("oldPositionX"), OldX);
    J->SetNumberField(TEXT("oldPositionY"), OldY);
...
TSharedPtr<FJsonObject> FMoveBlueprintNodeTool::BuildInputSchema() const
{
    ...
    P->SetObjectField(TEXT("graphName"), BuildStringProperty(TEXT("Stable or unique graph name.")));
    ...
    P->SetObjectField(TEXT("horizontalSpacing"), I);
    P->SetObjectField(TEXT("verticalOffset"), I);
    ...
    TArray<TSharedPtr<FJsonValue>> Req{ MakeShared<FJsonValueString>(TEXT("objectPath")), MakeShared<FJsonValueString>(TEXT("nodeGuid")) };
    S->SetArrayField(TEXT("required"), Req);
    return S;
}
