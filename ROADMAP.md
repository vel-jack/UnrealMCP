...
Not yet covered, and required before E2 can be called complete: injected-failure coverage for the compile-failure rollback branch (no fault-injection seam exists yet, so `graphRestored`/`dirtyRestored` are implemented but unexercised live), the mutation review across every exposed tool, and the disconnection/replay/expired-ID tests listed above.
