...
- While verifying Phase 5 live, a real adapter bug was found and worked around (not yet fixed in code): `UnrealSessionManager.GetMirroredToolListAsync` never refreshes its live tool catalog before an agent session's first `tools/list` call, so a newly registered native tool stays invisible to a connected agent no matter how many times the session restarts. Manually reseeding the adapter's on-disk cache (`%LocalAppData%\UnrealMCP\cache\unreal-tools.json`) from a live `ListTools` result unblocked two stuck sessions immediately, but this is a one-time manual workaround, not a fix. See the new checklist item under Milestone 20's Universal MCP Distribution.

## Priority Timeline

1. Fix the adapter tool-catalog staleness bug found while verifying Phase 5 (`UnrealSessionManager.GetMirroredToolListAsync` never attaches before an agent's first `tools/list`, so newly registered native tools stay invisible until someone manually reseeds the on-disk cache). See Milestone 20's Universal MCP Distribution checklist for the exact fix options.
...
3. Add Phase 4D automation coverage for the vector-delta/group-transform graphs once Phase 4C above lands, plus a formal `UnrealMCP.*` Live test for Phase 5's Enhanced Input Asset Authoring tools (currently live-verified manually through the adapter, not yet covered by an automated regression test).
...
5. Add Milestone 12 C++ build/hot-reload feedback (`BuildModule`, `GetCompilerErrors`) and Milestone 14's `InspectCppBlueprintAPI` — both were manually substituted for this session (shelling out to a build tool directly, hand-reading engine headers) while building Phase 5, confirming real value ahead of their prior position in this list.
6. Extend Milestone 10 with focused project-health diagnostics while preserving its completed architecture-analysis core.
7. Add Milestone 11 structured logging/diagnostics.
8. Add only the restricted Milestone 13 runtime-evidence/session controls needed by explicit deterministic tests, followed by Milestone 15 reusable test evidence and history.
9. Implement the rest of Milestone 14 Blueprint-to-C++ migration planning, rewiring, and equivalence validation.
10. Continue with advanced editor systems, source control, performance, and documentation.
...
Add the missing non-Blueprint data-asset authoring required to wire new gameplay input without hand-editing assets outside MCP. Implemented and live-verified; surfaced by the AR_GIS_MAP GIS globe navigation feature, which needed two new `InputAction` assets and a remap of an existing `InputMappingContext`'s key bindings that no existing tool could perform.
...
- `AddInputMappingContextMapping` adds one key-mapping row to an existing `InputMappingContext`: target `InputAction` asset, an `FKey`, and optional triggers/modifiers. Idempotent if the identical row already exists.
...
- Do not hardcode AR_GIS_MAP asset names (`IA_GIS_OrbitDrag`, `IMC_GIS_3D`, and similar) into these tools; they must stay project-agnostic.
...
- Phase 5 Enhanced Input Asset Authoring is implemented and live-verified. `CreateInputAction`, `CreateInputMappingContext`, `AddInputMappingContextMapping`, `RemoveInputMappingContextMapping`, and `GetInputMappingContextMappings` are implemented in `Source/UnrealMCP/{Public,Private}/Tools/` and registered in `UnrealMCPModule.cpp`. `CreateInputAction`/`CreateInputMappingContext` follow the `CreateBlueprintAsset` pattern (reject overwrite, dry-run, optional save). `AddInputMappingContextMapping`/`RemoveInputMappingContextMapping` do not hand-roll `TArray<FEnhancedActionKeyMapping>` mutation; they invoke `UInputMappingContext::MapKey`/`UnmapKey` through pure reflection (`FStructOnScope` sized from the live `UFunction`, parameters matched by property type/order rather than by name, `ProcessEvent` dispatch), so Epic's own mutation/validation logic runs unchanged. `GetInputMappingContextMappings` reads the protected `Mappings` array directly via `FArrayProperty`/`FScriptArrayHelper`, since protected `UPROPERTY`s are reflection-visible regardless of C++ access. None of the five tools added Enhanced Input as a hard module dependency; only `InputCore` was added to `UnrealMCP.Build.cs` (for `FKey`, which Engine already publicly depends on) — `UInputAction`/`UInputMappingContext` classes are resolved at runtime via `LoadClass<UObject>(nullptr, TEXT("/Script/EnhancedInput...."))`, matching `AddEnhancedInputActionNodeTool`'s existing convention. Native compilation passed clean (`AR_GIS_MAPEditor` Win64 Development, 35/35 actions, zero errors) against the AR_GIS_MAP host project on September 2, 2026. After an editor restart and the adapter-catalog workaround described below, all five tools were live-verified through the adapter against real `/Game/UnrealMCP_Test/IA_MCPTest_Action` and `IMC_MCPTest_Context` fixtures on September 3, 2026: dry-run and real `CreateInputAction`/`CreateInputMappingContext`; `AddInputMappingContextMapping` add then idempotent re-add (`alreadyExists=true, added=false`, confirmed no duplicate row); `GetInputMappingContextMappings` matching exactly both before and after; `RemoveInputMappingContextMapping` removing the row and echoing it back, then correctly rejecting a second removal as zero-match; and a dry-run add with an invalid `FKey` name correctly rejected (`'NotARealKeyName' is not a recognized FKey name.`). No formal `UnrealMCP.*` automation test exists yet for this phase — that gap is tracked in the Priority Timeline, matching the precedent already accepted for `RefreshBlueprintCallSites`.
- Verifying Phase 5 live surfaced a genuine adapter bug, tracked under Milestone 20's Universal MCP Distribution checklist rather than here: `UnrealSessionManager.GetMirroredToolListAsync` only attempts a live refresh when `_activeProject` is already set, but that is only ever set by an in-session attach call, which necessarily happens after the MCP client's one and only `tools/list` handshake at session start. So a newly registered native tool can never reach a connected agent's tool manifest through normal usage, no matter how many times the session is restarted — confirmed live across two independent agent sessions, both of which could see the five new tools via an in-session `ListTools` call after attaching, while their own MCP-level tool-calling surface (populated by `tools/list` before that attach) had none of them, and attaching mid-session did not retroactively fix it. Manually overwriting the adapter's on-disk cache (`%LocalAppData%\UnrealMCP\cache\unreal-tools.json`) with a fresh live `ListTools` result did unblock a subsequently restarted session, which is how Phase 5's live verification above was ultimately able to proceed — but this is a one-time manual workaround, not a code fix, and must not be read as resolving the underlying bug.
...
- Not started
- `InspectCppBlueprintAPI` will also be useful outside migration: after a project adds a new native class (such as AR_GIS_MAP's GIS globe navigation component/library), it would confirm the class is actually reflected and Blueprint-callable, and report its exact signature, before an agent wires Blueprint nodes to it.
...
- [ ] **Bug, not just a missing nicety**: `UnrealSessionManager.GetMirroredToolListAsync` only attempts a live Unreal refresh `if (_activeProject is not null)` (`UnrealSessionManager.cs` ~line 358), but `_activeProject` is only ever set by an in-session `SelectProject`/`AttachToUnrealProject` call, which necessarily happens *after* the MCP client's one-time `tools/list` handshake at session start (an agent cannot call a tool before its session exists). So `_activeProject` is always null at the exact moment `tools/list` fires, the live refresh is always skipped, and `McpRequestDispatcher.GetToolDefinitionsAsync` falls back to `ToolCatalogService`'s on-disk cache — which itself is never written, because no session has ever been attached *before* its own `tools/list` call fires, so `_toolCatalog.Save()` never runs with fresh data either. Net effect: a newly registered native tool can never reach a connected agent's tool manifest through normal usage, no matter how many times the agent session is restarted. Confirmed live on September 2, 2026, after Milestone 9 Phase 5 landed: `ListTools` against the running editor (a live in-session call, made *after* attaching) correctly reported 115 tools including all 5 new ones, while the same session's own MCP-level tool-calling surface (populated by `tools/list` before that attach) had none of them, and attaching mid-session did not retroactively fix it. A real fix needs the adapter to either eagerly auto-attach to the single discoverable project before its first `tools/list` response when unambiguous, or add an MCP `notifications/tools/list_changed` push plus an in-session "refresh tool manifest" action once *any* attach succeeds, so a live-but-late attach can still update an already-connected client without a restart.
